...
function handleReject(error, config) {
    ...
    if (axios.isCancel(error) || (error && errorMsg === 'Request aborted')) {
        return Promise.reject(error)
    }
    if (error.response) {
        const { status, data } = error.response
        let message = (error && errorMsg) || (data && data.message)
        if (status === 401) {
            message = '登录异常'
        } else if (status === 500) {
            message = '系统出现异常'
    ...
    } else if (errorCode === 'ECONNABORTED' || errorMsg.indexOf('timeout') !== -1) {
        console.log('请求超时')
        $error('请求超时')
    } else if (errorMsg.indexOf('Network Error') !== -1) {
        console.log('网络错误')
        $error('网络错误')
    } else if (errorMsg && config.globalError) {
